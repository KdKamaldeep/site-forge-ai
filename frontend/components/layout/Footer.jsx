'use client';

/**
 * Editorial Blog Footer Component
 * Multi-column footer matching design
 */

import { useEffect, useState } from 'react';
import Link from 'next/link';
import Image from 'next/image';
import { formatTitle } from '@/lib/textFormat';
import { getStandalonePages } from '@/lib/api';
import { normalizeThumbnail, normalizeImageUrl } from '@/lib/imageUtils';
import styles from './Footer.module.css';

export default function Footer({ navigation, tenant, categories = [], popularPosts = [] }) {
  const [standalonePages, setStandalonePages] = useState([]);
  const currentYear = new Date().getFullYear();
  const brandName = tenant?.brandIdentity?.brandName || tenant?.name || 'Site';
  const formattedBrandName = brandName.split(' ').map(word => 
    word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
  ).join(' ');

  // Fetch standalone pages for footer
  useEffect(() => {
    async function fetchStandalonePages() {
      if (!tenant?._id) return;
      
      try {
        const pages = await getStandalonePages(tenant._id);
        if (pages && Array.isArray(pages)) {
          setStandalonePages(pages);
        }
      } catch (error) {
        console.error('Error fetching standalone pages:', error);
      }
    }
    
    fetchStandalonePages();
  }, [tenant?._id]);

  return (
    <footer className={styles.mainFooter}>
      <div className={styles.footerContent}>
        {/* Left Column - Branding & Social */}
        <div className={styles.footerColumn}>
          <h3 className={styles.footerLogo}>{formattedBrandName}</h3>
          <p className={styles.footerCopyright}>
            © {currentYear} {formattedBrandName} | Made by LavanyaVerse Team
          </p>
          {/* Only render social icons if tenant has valid social links configured */}
          {(() => {
            const socialLinks = tenant?.settings?.socialLinks || [];
            // Check if we have any valid social links (not empty, not just "#")
            const hasValidSocialLinks = Array.isArray(socialLinks) && 
              socialLinks.some(link => link && typeof link === 'string' && link.trim() !== '' && link !== '#');
            
            if (!hasValidSocialLinks) {
              return null; // Don't render social icons section if no valid links
            }
            
            // Map social links to icons (assuming order: Facebook, Twitter, Pinterest, YouTube, Instagram)
            // If we have fewer links than icons, only render the ones we have
            const socialPlatforms = [
              { label: 'Facebook', color: '#1877f2', path: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z' },
              { label: 'Twitter', color: '#1da1f2', path: 'M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z' },
              { label: 'Pinterest', color: '#e60023', path: 'M12 0C5.373 0 0 5.372 0 12s5.373 12 12 12c5.302 0 9.917-3.158 11.877-7.68-.087-1.016-.528-3.917-1.26-5.8-.57 1.016-2.243 2.12-3.28 2.558-.63.303-1.22.457-1.78.457-1.5 0-2.5-1.1-2.5-2.5 0-1.9 1.4-3.5 3.2-3.5 1.8 0 2.7 1.3 2.7 2.9 0 1.1-.4 2.1-.9 2.7-.1.2-.2.3-.1.5.1.2.3.6.4.8.1.2.2.3.3.2.5-.6 1.1-1.4 1.4-2.3.5-1.5.2-3.1-.6-4.3-1.1-1.5-3-2.4-4.8-2.4-3.3 0-5.4 2.4-5.4 5.5 0 1.1.4 2.3 1 3.1.1.2.1.3.1.4-.1.4-.3 1.2-.4 1.4-.1.3-.3.4-.6.2-1.6-.7-2.6-2.9-2.6-4.7 0-3.8 2.8-7.2 7.5-7.2 3.9 0 6.9 2.8 6.9 6.5 0 3.9-2.5 7-6 7-1.2 0-2.3-.6-2.7-1.4 0 0-.6 2.3-.7 2.8-.3 1.1-1 2.5-1.5 3.5C9.4 23 10.6 23.5 12 23.5c6.627 0 12-5.373 12-12S18.627 0 12 0z' },
              { label: 'YouTube', color: '#ff0000', path: 'M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z' },
              { label: 'Instagram', color: 'linear-gradient(45deg, #f09433 0%,#e6683c 25%,#dc2743 50%,#cc2366 75%,#bc1888 100%)', path: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z' },
            ];
            
            return (
              <div className={styles.footerSocial}>
                {socialPlatforms.slice(0, Math.min(socialLinks.length, socialPlatforms.length)).map((platform, index) => {
                  const linkUrl = socialLinks[index];
                  // Only render if link is valid (not "#", not empty)
                  if (!linkUrl || linkUrl === '#' || linkUrl.trim() === '') {
                    return null;
                  }
                  return (
                    <a 
                      key={index}
                      href={linkUrl} 
                      aria-label={platform.label} 
                      className={styles.socialIcon} 
                      style={{ 
                        backgroundColor: platform.color.startsWith('linear-gradient') ? undefined : platform.color,
                        background: platform.color.startsWith('linear-gradient') ? platform.color : undefined
                      }}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                        <path d={platform.path}/>
                      </svg>
                    </a>
                  );
                })}
              </div>
            );
          })()}
        </div>

        {/* Useful Links - Standalone Pages */}
        <div className={styles.footerColumn}>
          <h4 className={styles.footerHeading}>Useful Links</h4>
          <ul className={styles.footerLinks}>
            {standalonePages.length > 0 ? (
              standalonePages.map((page) => (
                <li key={page._id}>
                  <Link href={`/${page.slug}`}>{formatTitle(page.title)}</Link>
                </li>
              ))
            ) : (
              // Fallback if no standalone pages are available yet
              <>
                <li><Link href="/about-us">About Us</Link></li>
                <li><Link href="/contact">Contact</Link></li>
                <li><Link href="/privacy-policy">Privacy Policy</Link></li>
                <li><Link href="/cookie-disclosure">Cookie Disclosure</Link></li>
              </>
            )}
          </ul>
        </div>

        {/* Popular Posts */}
        <div className={styles.footerColumn}>
          <h4 className={styles.footerHeading}>Popular Posts</h4>
          <div className={styles.popularPostsList}>
            {popularPosts.length > 0 ? (
              popularPosts.map((post) => (
                <Link key={post._id} href={post.categoryKey ? `/${post.categoryKey}/${post.slug}` : `/${post.slug}`} className={styles.popularPost}>
                  <div className={styles.popularThumbnail}>
                    {(() => {
                      const normalizedThumbnail = normalizeThumbnail(post.thumbnail);
                      const normalizedOgImage = normalizeImageUrl(post.meta?.ogImage);
                      return normalizedThumbnail?.url ? (
                        <Image
                          src={normalizedThumbnail.url}
                          alt={post.title}
                          fill
                          sizes="80px"
                          quality={70}
                          className={styles.popularImage}
                          style={{ objectFit: 'cover' }}
                        />
                      ) : normalizedOgImage ? (
                        <Image
                          src={normalizedOgImage}
                          alt={post.title}
                          fill
                          sizes="80px"
                          quality={70}
                          className={styles.popularImage}
                          style={{ objectFit: 'cover' }}
                        />
                      ) : (
                        <div className={styles.popularPlaceholder}></div>
                      );
                    })()}
                  </div>
                  <div className={styles.popularContent}>
                    <h5 className={styles.popularTitle}>{formatTitle(post.title)}</h5>
                    <div className={styles.popularMeta}>
                      BY {post.meta?.author?.name?.toUpperCase() || 'LavanyaVerse'} -{' '}
                      {post.updatedAt && new Date(post.updatedAt).toLocaleDateString('en-US', { 
                        month: 'short', 
                        day: 'numeric', 
                        year: 'numeric'
                      }).toUpperCase()}
                    </div>
                  </div>
                </Link>
              ))
            ) : (
              <p className={styles.noPosts}>No popular posts available</p>
            )}
          </div>
        </div>

        {/* Newsletter */}
        <div className={styles.footerColumn}>
          <h4 className={styles.footerHeading}>Newsletter</h4>
          <div className={styles.newsletterSection}>
            <label className={styles.checkboxLabel}>
              <input type="checkbox" className={styles.checkbox} />
              <span>Subscribe for our daily news</span>
            </label>
            <div className={styles.newsletterForm}>
              <input
                type="email"
                placeholder="Your email"
                className={styles.newsletterInput}
              />
              <button className={styles.newsletterButton}>Sign Up</button>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
